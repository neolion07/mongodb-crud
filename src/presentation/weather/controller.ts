...
export class WeatherController{
    ...
    create = (req:Request, res:Response) => {
        ...
        .catch(error => res.status(500).json({error: "at weatherService.create"}));
    }
    ...
    update = (req:Request, res:Response) => {
        ...
        const [
        	error,
        	updateWeatherDto
        ] = UpdateWeatherDto.update({...req.body});
        if(error) return res.status(400).json({error});
        ...
        .catch(error => res.status(500).json({error: "at weatherService.update"}));
    }
